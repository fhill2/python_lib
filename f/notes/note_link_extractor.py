...
# linkparser_extract.py:
# lower level: parses every markdown file and returns links in the form Visit(url=url, title=title)
...
class LinkExtractor:
    ...
    def extract(self, fname: PathIsh) -> Iterator[Extraction]:
        """the main function to execute to extract urls / links from a markdown/text file"""
        """fname = filepath"""
        path = Path(fname)
        fallback_dt = file_mtime(path)

        p = LinkExtractor(path)
        for r in p.walk():
